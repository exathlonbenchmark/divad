...
train_window_model_id=$(get_train_window_model_id "$detector_name" "$signature_lengths" "$filters" "$kernel_sizes" \
"$strides" "$optimizer" "$adamw_weight_decay" "$learning_rate" "$batch_size" "$n_epochs" "$early_stopping_target" \
...
for scores_avg_beta in 0 0.8 0.9 0.95 0.96667 0.975 0.98 0.98333 0.9875 0.99167 0.99375 0.995; do
  ...
  detector.train_window_model.signature_lengths="$signature_lengths" \
  detector.train_window_model.filters="$filters" \
  detector.train_window_model.kernel_sizes="$kernel_sizes" \
  detector.train_window_model.strides="$strides" \
